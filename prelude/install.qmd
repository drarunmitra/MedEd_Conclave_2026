...
Install the **current release, 4.6.x**. The interactive primers run R 4.6 in your browser, so a matching local version means the same code behaves the same way in both places.

The hard floor is **4.1**: below that the native pipe `|>` does not exist, and we use it everywhere. Anything from 4.1 up will get you through the two days, so do not let an upgrade block you if you are short of time or admin rights.
