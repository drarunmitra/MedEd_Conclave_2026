...
In the **Console** pane (bottom left), type each line and press <kbd>Enter</kbd>:
...
If `quarto::quarto_version()` errors, run `install.packages("quarto")` first. If it still fails, RStudio cannot see Quarto on your `PATH`. Restart your computer (genuinely, this fixes it most of the time), then see [Troubleshooting](../resources/troubleshooting.qmd).
