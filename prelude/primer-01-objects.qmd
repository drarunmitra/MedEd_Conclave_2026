...
#| exercise: ex_count
#| check: true
if (identical(as.numeric(.result), 4)) {
  list(correct = TRUE, message = "Correct — the answer is 4: the marks 78, 70, 91 and 83 are all 70 or above. One caution, because this check can only see your answer and not your code: other comparisons also happen to give 4 on these five marks, so satisfy yourself that you wrote osce >= 70. The >= is what keeps the student who scored exactly 70.")
} else if (identical(as.numeric(.result), 3)) {
  list(correct = FALSE, message = "Not quite — you have 3, and the answer is 4. The usual cause is > where you need >=: osce > 70 drops the student who scored exactly 70.")
} else {
